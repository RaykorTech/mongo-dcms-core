...
var core = module.exports = {
  connect: function (connectionUrl) {
    ...
    mongo.MongoClient.connect(connectionUrl, function (err, db) {
      if (err) throw new Error('fail to connect to database ' + connectionUrl)
      gfs = Grid(db, mongo)
  ...
  uploadFile: function (document, additionalMetadata, cb) {
    if (!gfs) {
      preConnectionOperationQueue.push({
        method: 'uploadFile',
        args: arguments
      })
      return
  ...
  getFileContentByFileId: function (fileId, cb) {
    if (!gfs) {
      preConnectionOperationQueue.push({ method: 'getFileContentByFileId', args: arguments })
      return
    }
    if (!fileId) {
